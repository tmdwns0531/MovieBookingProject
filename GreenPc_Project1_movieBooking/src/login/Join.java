...
class Join extends JPanel {
	...
	Join() {
		...
		checkEmale.addActionListener((event) -> {
			...
			if (PostMan.sendCodeToEmail(tfEMAIL.getText()) == 0) {
				JOptionPane.showMessageDialog(null, "ÀÎÁõ¹øÈ£¸¦ ¹ß¼ÛÇß½À´Ï´Ù", "check success", JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(null, "ÀÎÁõ¹øÈ£¹ß¼Û ½ÇÆÐ *°³¹ßÀÚ ¹®ÀÇ*", "check fail", JOptionPane.ERROR_MESSAGE);
			}
			System.out.println("ÀÌ¸ÞÀÏ ÄÚµåÀÔ·Â : " + PostMan.getCode());
		});
	}
	...
	int checkingInfo(String[] list) {
		...
		if (tfAuthCode.getText().length() == 0) {
			JOptionPane.showMessageDialog(null, "ÀÎÁõ¹øÈ£¸¦ ÀÔ·ÂÇÏ¼¼¿ä.", "check fail", JOptionPane.ERROR_MESSAGE);
			tfEMAIL.grabFocus();
			return -1;
		} else if (tfAuthCode.getText().contains(" ")) {
			JOptionPane.showMessageDialog(null, "ÀÎÁõ¹øÈ£¿¡ °ø¹éÀÌ Æ÷ÇÔµÇ¾ú½À´Ï´Ù", "check fail", JOptionPane.ERROR_MESSAGE);
			tfEMAIL.grabFocus();
			return -1;
		}
		...
		// ÀÌ¸ÞÀÏ ÄÚµå ÀÎÁõ
		if (authEmailCode() == -1) {
			JOptionPane.showMessageDialog(null, "ÀÌ¸ÞÀÏ ÀÎÁõ¹øÈ£°¡ ÀÏÄ¡ÇÏÁö ¾Ê½À´Ï´Ù", "check fail", JOptionPane.ERROR_MESSAGE);
